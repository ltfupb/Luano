...
└── .github/workflows/
    ...
    └── build.yml                # CI duplicate (실제 빌드는 luano-build repo)
```
...
- Feature gate: `electron/pro/index.ts`에서 LemonSqueezy 라이센스 또는 `@luano/pro` 패키지 또는 `LUANO_PRO=1` 환경변수로 판별
...
- `build.yml`: ci.yml과 동일 (중복, 정리 예정)

릴리즈 빌드 (private `luano-build` repo):
- `build.yml`: public repo checkout + Pro overlay → Win/Mac/Linux 3-platform 빌드 → public repo에 GitHub Release 발행
- `v*` 태그 푸시 시 자동 트리거
